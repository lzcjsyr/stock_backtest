...
**核心原则**：所有可调节参数必须暴露，禁止硬编码，提供友好的用户配置界面。
...
- **策略概览工作表 (Strategy_Overview)**
  ...
  - 关键参数设置及说明
  - 回测时间范围
  - 核心业绩指标汇总（总收益率、年化收益率、最大回撤、夏普比率、胜率等）
  - 盈亏决策周期数量统计（如：7次亏损，5次盈利）
  - 连续亏损/盈利最大周期数统计
...
**目标**：对策略、参数和结果进行深度总结和展示，重点在于分析而非详细列举。
